...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: isDocumentId(doc) ? `History — ${DOCUMENTS[doc].label}` : "Not found",
  };
}
...
export default async function HistoryPage({ params }: Props) {
  ...
  const [{ data: current }, { data: revisions, error }] = await Promise.all([
    state.supabase
      .from("cms_documents")
      .select("version, updated_at, updated_by_email")
      .eq("id", doc)
      .maybeSingle(),
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-5 py-8 lg:px-10 lg:py-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p
              ...
              style={{ color: "var(--color-admin-fg-muted)" }}
            >
              Earlier versions of{" "}
              <strong>{DOCUMENTS[doc].label}</strong>. Restoring one makes it
              live again; the version it replaces is kept here too, so nothing
              is lost.
            </p>
          </div>
        ...
        <ol
          className={`${card} mt-8 divide-y divide-[var(--color-admin-border-soft)]`}
        >
          {current ? (
